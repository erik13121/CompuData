...
namespace CompuData.Controllers
{
    public class AccessLevelsController : Controller
    {
        ...
        {
            AccessLevel myModel = new AccessLevel();
            if (TempData["model"] != null)
            {
                myModel = (AccessLevel)TempData["model"];
        ...
        [HttpPost]
        public ActionResult Delete(string levelID)
        {
            try
            {
                var db = new CodeFirst.CodeFirst();
                var intLevelID = int.Parse(levelID);
                var level = db.Access_Level.Where(t => t.AccessLevelID == intLevelID).FirstOrDefault();
                db.Access_Level.Remove(level);
                db.SaveChanges();

                var redirectUrl = new UrlHelper(Request.RequestContext).Action("Index", "AccessLevels");
